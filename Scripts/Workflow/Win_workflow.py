...
def Workflow(ds, path):
    os.makedirs(path + r'\clean\')
    os.makedirs(path + r'\import\')
    search(path + r'\import\', ds)
    clean_h(path + r'\import\0_homology_genes.tsv', path + r'\clean\0_homology_genes.tsv')
    clean_g(path + r'\clean\r', path + r'\import\')
    click.echo('Done')